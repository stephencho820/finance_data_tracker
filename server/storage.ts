...
export interface IStorage {
  ...
  getDailyStockDataByMarketAndRank(
    market: string,
    rankType: string,
    date: string,
  ...
  createManyDailyStockData(
    data: InsertDailyStockData[],
  ): Promise<DailyStockData[]>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  async createStockData(insertData: InsertStockData): Promise<StockData> {
    const [data] = await db.insert(stockData).values(insertData).returning();
    return data;
  }

  async createManyStockData(
    insertData: InsertStockData[],
  ): Promise<StockData[]> {
  ...
  async getAllStockData(): Promise<StockData[]> {
    const data = await db
      .select()
      .from(stockData)
      .orderBy(desc(stockData.createdAt));
    return data;
  }
  ...
  async getDailyStockDataBySymbol(symbol: string): Promise<DailyStockData[]> {
    const result = await db
      .select()
      .from(dailyStockData)
      .where(eq(dailyStockData.symbol, symbol))
      .orderBy(desc(dailyStockData.date));
    return result;
  }
  ...
  async getDailyStockDataByDate(date: string): Promise<DailyStockData[]> {
    const result = await db
      .select()
      .from(dailyStockData)
      .where(eq(dailyStockData.date, date));
    return result;
  }

  async getDailyStockDataByMarketAndRank(
    market: string,
    rankType: string,
    date: string,
  ): Promise<DailyStockData[]> {
  ...
  async createDailyStockData(
    insertData: InsertDailyStockData,
  ): Promise<DailyStockData> {
    const [data] = await db
  ...
  async createManyDailyStockData(
    insertData: InsertDailyStockData[],
  ): Promise<DailyStockData[]> {
    if (!insertData || insertData.length === 0) {
      return [];
    }
    const data = await db.insert(dailyStockData).values(insertData).returning();
    return data;
  }
