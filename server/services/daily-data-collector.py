...
"""
5년치 일일 데이터 수집 스크립트
매일 오전 8시에 실행되어 KOSPI/KOSDAQ 시가총액/거래량 top 500 종목의 데이터를 수집
"""
...
def collect_historical_data(symbols, market, start_date, end_date):
    ...
    try:
        ...
        print(f"[INFO] Collecting historical data for {len(symbols)} symbols", file=sys.stderr)
        ...
        for i, stock_info in enumerate(symbols):
            ...
            try:
                # OHLCV 데이터 가져오기
                ohlcv_df = pykrx_stock.get_market_ohlcv_by_date(start_date_fmt, end_date_fmt, symbol)
                ...
                if not ohlcv_df.empty:
                    ...
                    for date_str, row in ohlcv_df.iterrows():
                        try:
                            ...
                            market_cap = None
                            try:
                                cap_df = pykrx_stock.get_market_cap_by_date(date_fmt, date_fmt, symbol)
                                if not cap_df.empty:
                ...
                # 진행률 출력
                if (i + 1) % 10 == 0:
                    print(f"[INFO] Processed {i + 1}/{len(symbols)} symbols", file=sys.stderr)
                    
            except Exception as e:
                print(f"[WARNING] Failed to collect data for {symbol}: {e}", file=sys.stderr)
                continue
        
        return collected_data
        
    except Exception as e:
        print(f"[ERROR] Historical data collection failed: {e}", file=sys.stderr)
        return []
...
def save_to_database(data, conn):
    ...
    try:
        ...
        for i in range(0, len(data), batch_size):
            ...
            print(f"[INFO] Saved {total_saved}/{len(data)} records", file=sys.stderr)
...
def log_collection_result(market, total_stocks, status, error_message, execution_time, conn):
    """수집 결과 로그 저장"""
    try:
        cursor = conn.cursor()
...
def main():
    ...
    try:
        # 입력 파라미터 파싱
        input_data = json.loads(sys.stdin.read()) if sys.stdin.isatty() == False else {"market": "KOSPI"}
        market = input_data.get('market', 'KOSPI')
        
        print(f"[INFO] Starting daily data collection for {market}", file=sys.stderr)
        ...
        print(f"[INFO] Collection period: {start_date.date()} to {end_date.date()}", file=sys.stderr)
        ...
        print(f"[INFO] Found {len(top_500_stocks)} top stocks", file=sys.stderr)
        
        # 5년치 데이터 수집
        historical_data = collect_historical_data(top_500_stocks, market, start_date, end_date)
        if not historical_data:
            raise Exception("Failed to collect historical data")
        
        print(f"[INFO] Collected {len(historical_data)} historical records", file=sys.stderr)
        ...
        # 성공 로그 저장
        log_collection_result(market, saved_count, 'success', None, execution_time, conn)
        ...
        print(json.dumps({
            'success': True,
            'message': f'Successfully collected and saved {saved_count} records for {market}',
            'data': {
                'market': market,
        ...
        if 'conn' in locals() and conn:
            log_collection_result(market, 0, 'failed', error_message, execution_time, conn)
            conn.close()
